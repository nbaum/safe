ready(function () {
    ...
    const eForget = the("#forget");
    const eEntries = the("#entries");
    const eName = the("#name");
    const eSafe = the("#safe");
    const eHost = the("#host");
    const eUsername = the("#username");
    const ePassword = the("#password");
    const eNotes = the("#notes");
    var link = null;
    ...
    function examineSituation() {
        chrome.tabs.query({ currentWindow: true, active: true }, function (tabs) {
            ...
            eEntries.click();
            chrome.tabs.sendMessage(tab.id, { action: "checkInject", args: theEntry }, function (result) {
                the("#inject").disabled = !result;
            });
